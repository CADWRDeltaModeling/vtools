...
class TestMerge(unittest.TestCase):
    ...
    def test_merge_rts_no_intersect(self):
        ...
        self.assertTrue(spy.alltrue(spy.isnan(nt.data[s1+1:s2])))
        self.assertTrue(spy.allclose(nt.data[0:s1+1],ts1.data))
        self.assertTrue(spy.allclose(nt.data[s2:len(nt.data)],ts2.data))
    ...
    def test_merge_rts_intersect(self):
        ...
        self.assertTrue(spy.allclose(nt.data[0:ii+1],ts1.data[0:ii+1]))
        self.assertTrue(spy.allclose(nt.data[ii+1:ii+num_nan+2],ts2.data[0:num_nan+1]))
        self.assertTrue(spy.allclose(nt.data[ii+num_nan+2:s1+1],ts1.data[ii+num_nan+2:s1+1]))
        self.assertTrue(spy.allclose(nt.data[s1+1:len(nt.data)],ts2.data[s3+1:len(ts2.data)]))
    ...
    def test_merge_large_rts(self):
        ...
        data1=list(range(0,largenum))
        dt=datetime.timedelta(minutes=15)
        rt1=rts(data1,st1,dt,{})

        st2=rt1.end+dt
        data2=list(range(largenum,2*largenum))
        rt2=rts(data2,st2,dt,{})

        st3=rt2.end+dt
        data3=list(range(2*largenum,3*largenum))
        rt3=rts(data3,st3,dt,{})

        st4=rt3.end+dt
        data4=list(range(3*largenum,4*largenum))
