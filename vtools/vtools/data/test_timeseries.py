...
class TestTimeSeries(unittest.TestCase):
    ...
    def testCompare(self):
        assert not self.ts2==None
        ts5 = self.ts1 > self.ts2
        self.assertEqual(ts5[4].value,True)
        
        assert numpy.isnan(ts5[3].value)
        ts5 = self.ts1 < self.ts2

        assert not ts5[4].value
        ts5 = (self.ts1 == self.ts2)
        self.assertEqual(ts5[4].value,False) 
        assert numpy.isnan(ts5[3].value)
        ts5 = (self.ts1 != self.ts2)
        self.assertEqual(ts5[4].value,True) 
        assert numpy.isnan(ts5[3].value)
        ts5 = (self.ts1 >= 3. )
        self.assertEqual(ts5[2].value,False) 
        self.assertEqual(ts5[3].value,True)         
        ts5 = (self.ts1 <= 3. )
        self.assertEqual(ts5[2].value,True) 
        self.assertEqual(ts5[3].value,True)   
        self.assertEqual(ts5[4].value,False) 
        ts5 = (self.ts1 > 4.) & (self.ts2 >= 4.)
    
        #assert numpy.isnan(ts5[3].value) 
        #self.assertEqual(ts5[4].value,False)         
        self.assertEqual(ts5[5].value,False) 
        self.assertEqual(ts5[6].value,True)         
