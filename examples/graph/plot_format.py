# Sample plot format for attributes you are likely to want to change
# There is an elaborate example, followed by lists of acceptable
# marker and line styles and colors
...
# Note the formatting parameters in this call
# to ts_plot could be set later as well
p0 = ts_plot(ts0,               # Series to plot
             "first series",    # Name, e.g. for legends
             plot_area=None,    # Background plot area. None means first plot in figure
             color="indianred", # Color of lines. See discussion of "color" below. Default black
             line_style="solid", # Default is a solid line, so this is unnecessary
             line_width=2,        # Make it a bit wider
             bgcolor="antiquewhite"
             )



p0 = ts_plot(resample(ts1,days(10)),
             "second series",
             plot_area=p0,     # Plot on top of previous one
             line_width=2,
             color="forestgreen",
             line_style="dash" # Available styles listed near end of file
             )

p0 = ts_plot(resample(ts3,days(20)),
             "third series",
             plot_area=p0,     # Plot on top of previous one
             line_width=2,
             color="lavenderblush",
             type="scatter", 
             marker="circle", # Available markers at bottom of file
             marker_size=2
             )


p0.title="Format Example"
p0.title_font = "decorative 28 bold" #See font info below
p0.legend.visible=True
p0.legend.font="roman"
p0.legend.bgcolor="cornsilk"
p0.legend.line_width=5
p0.y_axis.title="Y axis title"
p0.y_axis.title_font="modern 14 bold"
p0.y_axis.title_color="dimgrey"
p0.y_axis.tick_weight=2.0
p0.y_axis.tick_color="indianred"
p0.y_axis.tick_label_font="modern"
p0.y_axis.tick_label_color="indianred"
p0.y_axis.tick_in = 5 # how far the tick comes into plot area
p0.y_axis.tick_out = 2 # how far the tick goes out of plot area

#Change the date format on the x axis to eliminate time
p0.x_axis.tick_label_formatter.date_format="%d%b%Y"
p0.padding_bottom=40 # Reduce space in plot area below plot


#Set background color and make sure it applies in the 'padding'
#in the margins of the plot
p0.bgcolor="antiquewhite"
p0.fill_padding=True

             
# Save and display
save_image_file(p0,"plot_format.png")
...
# Trait line_style
# dash,dot dash,dot,long dash,solid

# Trait: marker
# Use   circle,cross,diamond,dot,inverted_triangle,
#       pixel,plus,square,triangle

# Trait: color (applies to lines, markers, plot backgrounds)
# Chaco works with either tuples of
#     RGB fractions (fractions of full saturation in
#                   red green and blue)
#     Predefined colors The map of predefined colors to
#                   RGB fractions is given below
# Please also see palette.py for an example using a
...
#Trait: font
# Fonts should be a string containing family, style and size info
# the words "pt" and "family" are ignored, but may make the code
# more reader
# e.g. '12 pt' 'swiss 16 italic bold' 'default 16' 'roman 24pt 
##font_families = [
##    'default',
##    'decorative',
##    'roman',
