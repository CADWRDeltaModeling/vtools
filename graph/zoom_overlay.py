...
from enthought.traits.api import Float, Instance, Int,Any
...
class ZoomOverlay(AbstractOverlay):
    ...
    def calculate_points(self, component):
        ...
        x_start=x_start+50+(float(component.bounds[0])-592)/200
        x_end=x_end+50+(float(component.bounds[0])-592)/200
        
        if x_start > x_end:
            x_start, x_end = x_end, x_start
        
        y_end = self.source.y+(component.bounds[1]-466)/2+284
        y_start = self.source.y2+(component.bounds[1]-466)/2+284
        ...
        y = self.destination.y2 + 1+50
        
        left_end = array([self.destination.x+50, y])
        right_end = array([self.destination.x2+50, y])
